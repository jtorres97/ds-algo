...
public:
		...
		, m_testType()
	{
		...
		m_cooldown = Utils::RandomDouble(m_minInterval, m_maxInterval);

		// Container for enum values
		m_testTypes = { PUSH, POP };
	}
	...
	void Use()
    {
		...
		m_testType = m_testTypes[Utils::RandomInt(0, 1)];

		// Variable cool downs for each test type
        switch (m_testType) 
		{
        	case PUSH:
				m_cooldown = Utils::RandomDouble(m_minInterval - 50.0, (m_maxInterval / 2.f) - 50.0);
        		break;
			case POP:
				m_cooldown = Utils::RandomDouble(m_minInterval + 300.0, m_maxInterval);
        		break;
        }
        ApplyCooldown();
    }
